...
export function calculateMyEdgeDeepAudit(
  ...
  for (const t of safeTrades) {
    const sess = getTradeSession(t, userTimezone);
  ...
  for (const [sess, cluster] of sessionMap.entries()) {
    sessionPerformances.push(analyzeCluster(cluster, sess, sess, 'Killzone'));
  }
  ...
  for (const t of safeTrades) {
    ...
    const session = getTradeSession(t, userTimezone);
  ...
  if (bestSession && bestSession.totalNetPnL > 0) {
    recurringConditions.push(
      `Killzone la plus rentable : ${bestSession.label} (P&L net ${bestSession.totalNetPnL >= 0 ? '+' : ''}${bestSession.totalNetPnL}).`
    );
  }
  ...
  if (topCombo && topCombo.sampleSize >= 2) {
    keyTakeaway = `Votre rentabilité maximale est concentrée sur ${topCombo.pair} pendant la killzone ${topCombo.session} (${topCombo.setup}), générant ${topCombo.winRate}% de win rate sur ${topCombo.sampleSize} trades.`;
  } else if (bestPair) {
