...
import { calculateRiskReward, isTradeRRComplete } from '../../calculations/riskReward';
...
export class TradeRepository {
  ...
  static async create(input: NewTradeInput, allowDuplicate = false): Promise<Trade> {
    ...
    // Auto-calculate RR ratios if not provided
    ...
    const plannedRR = input.plannedRR !== undefined && input.plannedRR !== null ? input.plannedRR : rrCalc.plannedRR;
    const realizedRR = input.realizedRR !== undefined && input.realizedRR !== null ? input.realizedRR : rrCalc.realizedRR;

    const tradeStatus = input.status || 'CLOSED';
    const isRRComplete = isTradeRRComplete({
      status: tradeStatus,
      entryPrice: input.entryPrice ?? null,
      stopLoss: input.stopLoss ?? null,
      takeProfit: input.takeProfit ?? null,
      exitPrice: input.exitPrice ?? null,
      isRRComplete: input.isRRComplete,
    });
    ...
    const candidateTrade: Trade = {
      ...
      takenInPostLossWindow: input.takenInPostLossWindow ?? false,
