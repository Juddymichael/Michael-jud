...
/**
 * Reference timezone for calendar day assignment.
 * Uses Europe/Paris (GMT+2 in Summer CEST / GMT+1 CET) as the institutional standard
 * aligning with European Forex / Killzones servers.
 */
export const DEFAULT_CALENDAR_TIMEZONE = 'Europe/Paris';

/**
 * Returns the calendar date string (YYYY-MM-DD) for any ISO date or timestamp,
 * formatted in the institutional reference timezone (Europe/Paris / GMT+2).
 * Strictly preserves Saturday and Sunday trades without rolling.
 */
 ...
 * Uses closedAt (fallback to openedAt), evaluated in the reference GMT+2 timezone.
 * Trades executed on Saturday or Sunday strictly remain on Saturday or Sunday.
 */
export function getTradeDateKey(
  trade: Trade,
  timeZone: string = DEFAULT_CALENDAR_TIMEZONE
): string {
  const dateStr = trade.closedAt || trade.openedAt;
  return getCalendarDateKey(dateStr, timeZone);
}
...
export function buildMonthCalendar(
  ...
  // Index trades by dateStr in the reference timezone
  const tradesByDate = new Map<string, Trade[]>();
  const monthTrades: Trade[] = [];

  for (const trade of closedTradesList) {
    const dKey = getTradeDateKey(trade, timeZone);
    if (!dKey) continue;
  ...
  // Split into weekly chunks (Monday to Sunday)
  ...
  for (let w = 0; w < calendarDays.length; w += 7) {
    ...
    for (const d of weekDays) {
      // Crucial : only accumulate days of the current month being viewed!
      // Days from previous/next month (grayed out) belong to their respective months
      // and must NOT alter the weekly totals of the currently viewed month.
 ...
 */
export function getAllTradingMonths(
  allTrades: Trade[],
  timeZone: string = DEFAULT_CALENDAR_TIMEZONE
): MonthTradeStats[] {
  ...
  return sortedKeys.map((key) => {
    ...
    return buildMonthCalendar(year, month, allTrades, timeZone);
  });
}
