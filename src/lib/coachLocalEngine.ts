...
${worst && worst.netPnL < 0 ? `2. Mettez en pause ou réduisez de 50% la taille de position sur **${worst.symbol}** jusqu'à identification des causes de slippage ou d'invalidation.` : ''}`;
  ...
  if (q.includes('killzone') || q.includes('session') || q.includes('horaire') || q.includes('londres') || q.includes('new york')) {
    ...
    if (validSessions.length === 0) {
      return `### **Fait statistique**
Données de Killzones insuffisantes ($n < 2$ trades par Killzone).

### **Interprétation**
Données insuffisantes pour conclure sur la Killzone optimale.

### **Action concrète**
Renseignez systématiquement la Killzone de trading (London Killzone 09h-12h, New York Killzone 14h-17h, Asian Killzone) lors de l'enregistrement de vos ordres.`;
    }
    ...
    return `### **Fait statistique**
- **Meilleure Killzone** : **${best.session}** (${best.sampleSize} trades, Win Rate **${formatPercent(best.winRate, 1)}**, P&L net **${best.netPnL >= 0 ? '+' : ''}${best.netPnL}€**).
${worst && worst.session !== best.session ? `- **Killzone la moins performante** : **${worst.session}** (${worst.sampleSize} trades, Win Rate **${formatPercent(worst.winRate, 1)}**, P&L net **${worst.netPnL >= 0 ? '+' : ''}${worst.netPnL}€**).` : ''}

### **Observation & Interprétation**
La Killzone **${best.session}** offre les expansions de prix et les livraisons institutionnelles les plus conformes à vos critères de timing d'entrée.

### **Action concrète**
Planifiez votre présence devant les écrans exclusivement durant la Killzone **${best.session}** et évitez les prises de position en fin de journée ou hors horaires de liquidité majeure.`;
  }
