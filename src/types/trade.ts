...
export interface Trade {
  ...
  plannedRR?: number | null; // Planned Risk/Reward ratio
  realizedRR?: number | null; // Realized Risk/Reward ratio based on exit price
  isRRComplete?: boolean; // True if trade has all mandatory RR fields for its status (OPEN: entry+SL+TP, CLOSED: entry+SL+exit)
  ...
  setup: string | null;
  setupId?: string | null;
  entryTrigger?: string | null; // Specific entry trigger (e.g. "M5 CHoCH confirmé")
  ...
  ifvg?: boolean | null;
  ob?: boolean | null;
  killzone?: 'LONDON_OPEN' | 'NY' | 'NY_AM' | 'NY_PM' | 'LONDON_CLOSE' | 'ASIA' | 'OFF_HOURS' | string | null;
  ...
  emotion: EmotionType;
  mistake: MistakeType;
  takenInPostLossWindow?: boolean; // Trade pris dans la fenêtre de vigilance post-perte
  warningAcknowledged?: boolean; // Confirmation explicite de respect du plan validée par l'utilisateur

  // Categorization
  tags: string[];

  // Attachments & Visual Proof
  tradingViewUrl?: string | null; // TradingView chart link
  screenshotBefore: string | null; // Chart screenshot at entry
  screenshotAfter: string | null; // Chart screenshot at exit
